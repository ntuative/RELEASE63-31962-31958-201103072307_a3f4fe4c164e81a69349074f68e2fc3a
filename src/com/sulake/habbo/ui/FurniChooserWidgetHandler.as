package com.sulake.habbo.ui
{
   ...
   public class FurniChooserWidgetHandler implements IRoomWidgetHandler
   {
      ...
      private function handleFurniChooserRequest() : void
      {
         ...
         var _loc3_:* = [];
         var _loc9_:int = this._container.roomEngine.getRoomObjectCount(_loc1_,_loc2_,RoomObjectCategoryEnum.const_29);
         _loc4_ = 0;
         while(_loc4_ < _loc9_)
         {
            _loc8_ = this._container.roomEngine.getRoomObjectWithIndex(_loc1_,_loc2_,_loc4_,RoomObjectCategoryEnum.const_29);
            if(_loc8_ != null)
            {
               _loc5_ = _loc8_.getModel().getNumber(RoomObjectVariableEnum.const_338);
         ...
         this._container.events.dispatchEvent(new RoomWidgetChooserContentEvent(RoomWidgetChooserContentEvent.const_733,_loc3_,this._container.sessionDataManager.isAnyRoomController));
      }
