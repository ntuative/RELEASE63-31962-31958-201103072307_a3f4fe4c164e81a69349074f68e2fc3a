package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_504:IKeyExchange;
      
      private var var_936:String;
      
      private var var_17:HabboLoginDemoView;
      
      private var var_2605:PseudoRandom;
      ...
      private var var_224:HabboHotelView;
      
      private var var_1430:String = "";
      
      private const const_1704:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_632 = param1;
            this.var_56.initConnection(HabboConnectionType.const_253);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_15.events.addEventListener(RoomSessionEvent.const_96,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_56 && this.var_160 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         this.var_224 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         var _loc1_:String = this.var_160.getKey("client.hotel_view.image");
         var _loc2_:String = this.var_160.getKey("image.library.url");
         if(_loc1_ != null && _loc2_ != null)
         {
            this.var_224.loadHotelViewImage(_loc2_ + _loc1_);
         }
         var _loc3_:* = this.var_160.getKey("use.sso.ticket") == "1";
         this.var_632 = this.var_160.getKey("sso.ticket",null);
         this.var_1858 = this.var_160.getKey("flash.client.url","");
         this.var_2607 = this.var_160.getKey("external.variables.txt","");
         if(this.var_224 == null)
         {
            this.var_224 = new HabboHotelView(this._windowManager,assets,Component(context).events);
         }
         if(_loc3_)
         {
            this.var_56.mode = HabboConnectionType.const_93;
            if(this.var_632)
            {
               this.var_56.initConnection(HabboConnectionType.const_253);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc4_)
            {
               this.var_17 = new HabboLoginDemoView(this);
               this.var_17.addEventListener(HabboLoginDemoView.const_535,this.onInitConnection);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_56.mode = HabboConnectionType.const_93;
            if(this.var_160.keyExists("local.environment"))
            {
               this.var_56.mode = HabboConnectionType.const_1220;
            }
         }
         this.var_56.initConnection(HabboConnectionType.const_253);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_897);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2605 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_56.mode)
         {
            case HabboConnectionType.const_93:
               _loc6_ = "";
               _loc6_ = this.var_160.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_160.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1430 = _loc4_;
               this.var_224.loadBannerImage(_loc7_ + "?token=" + this.var_1430,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1220:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1582:
               this.var_936 = this.generateRandomHexString(30);
               this.var_504 = new DiffieHellman(new BigInteger(this.var_160.getKey("connection.development.prime"),16),new BigInteger(this.var_160.getKey("connection.development.generator"),16));
               this.var_504.init(this.var_936);
               _loc8_ = this.var_504.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_504.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_504.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2605);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         this.var_1431 = true;
         this.dispatchLoginStepEvent(HabboCommunicationEvent.const_223);
         connection.send(new VersionCheckMessageComposer(401,this.var_1858,this.var_2607));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1704,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_632 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_632);
            _loc2_.send(_loc4_);
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_173);
         }
         else if(this.var_17 != null)
         {
            _loc5_ = this.var_17.name;
            _loc6_ = this.var_17.password;
      ...
      private function onIdentityAccounts(param1:IdentityAccountsEvent) : void
      {
         ...
         if(this.var_17)
         {
            this.var_17.populateUserList(param1.getParser().accounts);
         }
      }
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2606)
         {
            _loc2_ = this.var_160.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2606 = true;
         var _loc2_:String = this.var_160.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_160.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1430);
         ...
         this.var_504 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_504.init(_loc14_);
            _loc15_ = this.var_504.getPublicKey(10);
            if(_loc15_.length >= 64)
            {
               _loc12_ = _loc15_;
               this.var_936 = _loc14_;
            }
