package com.sulake.habbo.communication.messages.incoming.navigator
{
   import com.sulake.core.communication.messages.IMessageDataWrapper;
   import com.sulake.core.runtime.IDisposable;
   
   public class OfficialRoomEntryData implements IDisposable
   {
      
      public static const const_1027:int = 1;
      
      public static const const_786:int = 2;
      
      public static const const_858:int = 3;
      
      public static const const_1598:int = 4;
       
      
      private var _index:int;
      
      private var var_2273:String;
      
      private var var_2269:String;
      
      private var var_2271:Boolean;
      
      private var var_2275:String;
      
      private var var_951:String;
      
      private var var_2274:int;
      
      private var var_2272:int;
      
      private var _type:int;
      
      private var var_2270:String;
      
      private var var_867:GuestRoomData;
      
      private var var_868:PublicRoomData;
      
      private var _open:Boolean;
      
      private var _disposed:Boolean;
      
      public function OfficialRoomEntryData(param1:IMessageDataWrapper)
      {
         super();
         this._index = param1.readInteger();
         this.var_2273 = param1.readString();
         this.var_2269 = param1.readString();
         this.var_2271 = param1.readInteger() == 1;
         this.var_2275 = param1.readString();
         this.var_951 = param1.readString();
         this.var_2274 = param1.readInteger();
         this.var_2272 = param1.readInteger();
         this._type = param1.readInteger();
         if(this._type == const_1027)
         {
            this.var_2270 = param1.readString();
         }
         else if(this._type == const_858)
         {
            this.var_868 = new PublicRoomData(param1);
         }
         else if(this._type == const_786)
         {
            this.var_867 = new GuestRoomData(param1);
         }
         else
         {
            this._open = param1.readBoolean();
         }
      }
      
      public function dispose() : void
      {
         if(this._disposed)
         {
            return;
         }
         this._disposed = true;
         if(this.var_867 != null)
         {
            this.var_867.dispose();
            this.var_867 = null;
         }
         if(this.var_868 != null)
         {
            this.var_868.dispose();
            this.var_868 = null;
         }
      }
      
      public function get disposed() : Boolean
      {
         return this._disposed;
      }
      
      public function get type() : int
      {
         return this._type;
      }
      
      public function get index() : int
      {
         return this._index;
      }
      
      public function get popupCaption() : String
      {
         return this.var_2273;
      }
      
      public function get popupDesc() : String
      {
         return this.var_2269;
      }
      
      public function get showDetails() : Boolean
      {
         return this.var_2271;
      }
      
      public function get picText() : String
      {
         return this.var_2275;
      }
      
      public function get picRef() : String
      {
         return this.var_951;
      }
      
      public function get folderId() : int
      {
         return this.var_2274;
      }
      
      public function get tag() : String
      {
         return this.var_2270;
      }
      
      public function get userCount() : int
      {
         return this.var_2272;
      }
      
      public function get guestRoomData() : GuestRoomData
      {
         return this.var_867;
      }
      
      public function get publicRoomData() : PublicRoomData
      {
         return this.var_868;
      }
      
      public function get open() : Boolean
      {
         return this._open;
      }
      
      public function toggleOpen() : void
      {
         this._open = !this._open;
      }
      
      public function get maxUsers() : int
      {
         if(this.type == const_1027)
         {
            return 0;
         }
         if(this.type == const_786)
         {
            return this.var_867.maxUserCount;
         }
         if(this.type == const_858)
         {
            return this.var_868.maxUsers;
         }
         return 0;
      }
   }
}
