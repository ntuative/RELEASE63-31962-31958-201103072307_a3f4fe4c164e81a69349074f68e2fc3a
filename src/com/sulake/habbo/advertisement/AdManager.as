package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      {
         this.var_309 = param2 as IHabboCommunicationManager;
         if(this.var_309 == null)
         {
            return;
         }
         this._connection = this.var_309.getHabboMainConnection(this.onConnectionReady);
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2379 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2378 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2380 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1701 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
         ...
         if(_loc4_ != "" && _loc5_ != "")
         {
            ...
            _loc8_.addEventListener(AssetLoaderEvent.const_27,this.adWarningLeftReady);
            _loc7_ = new URLRequest(_loc5_);
            _loc8_ = assets.loadAssetFromFile("adWarningRight",_loc7_,"image/png");
            _loc8_.addEventListener(AssetLoaderEvent.const_27,this.adWarningRightReady);
         }
      }
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_212 != null && this.var_212.running)
         {
            this.var_212.reset();
            return false;
         }
         if(this.var_1117 == 0)
         {
            this.var_1117 = getTimer();
            return false;
         }
         if(this.var_1701 > 0 && this.var_1702 >= this.var_1701)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1117;
         if(_loc1_ > this.var_2378)
         {
            ++this.var_1702;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1361 = _loc2_.imageUrl;
         this.var_2377 = _loc2_.clickUrl;
         this.var_1117 = getTimer();
         if(this.var_1361 != "")
         {
            _loc3_ = this.var_1361;
            ...
            {
               this.var_212 = new Timer(this.var_2380,1);
               this.var_212.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_212.running)
            {
               this.var_212.reset();
            }
            this.var_212.start();
            _loc4_ = new URLRequest(this.var_1361);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
            _loc5_.addEventListener(AssetLoaderEvent.const_27,this.interstitialLoaderEventHandler);
            _loc5_.addEventListener(AssetLoaderEvent.const_46,this.interstitialLoaderEventHandler);
         }
         else
         {
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_503,0,0,param1.bitmapData,this.var_2377));
            this.var_1117 = getTimer();
         }
         else
         {
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1360 = _loc2_.imageUrl;
         this.var_1700 = _loc2_.clickUrl;
         if(this.var_1360 != "" && this.var_1700 != "")
         {
            _loc3_ = this.var_1360;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1360);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
            _loc5_.addEventListener(AssetLoaderEvent.const_27,this.roomAdImageReady);
         }
      }
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_553,0,0,_loc4_,this.var_1700,this.var_1118,this.var_1119));
         }
      }
