package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2505 = _loc3_.localizationId;
         this.var_379 = [];
         if(page && false && page.viewer.catalog)
         {
         ...
         events.addEventListener(WidgetEvent.const_775,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_379)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_514 < 0 || this.var_514 > this.var_379.length)
         {
            this.var_514 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2505)
         {
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_203 == null || this.var_786 >= this.var_203.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_514 >= 0 && this.var_514 < this.var_379.length)
         {
            _loc7_ = this.var_379[this.var_514] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_203[this.var_786] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_488,_loc3_,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_322,AvatarAction.const_684);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_514 >= this.var_379.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_379[this.var_514] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_203[this.var_786] as int;
