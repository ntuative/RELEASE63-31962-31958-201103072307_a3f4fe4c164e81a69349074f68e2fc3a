package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_1997:int = 100;
      
      private const const_1534:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_873 = new Timer(this.const_1997,1);
         this.var_873.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_407)
         {
            this.var_407.addEventListener(AssetLoaderEvent.const_27,this.onConfigurationComplete);
            this.var_407.addEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_407)
         {
            this.var_407.removeEventListener(AssetLoaderEvent.const_27,this.onConfigurationComplete);
            this.var_407.removeEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_871);
            if(this.var_1335.indexOf("?") > 0)
            {
               _loc2_ = this.var_1335 + "&retry=" + this.var_871;
            }
            else
            {
               _loc2_ = this.var_1335 + "?retry=" + this.var_871;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2310,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1096[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1096[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1676)
         {
            this.var_1095.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_872)
         {
            _loc7_ = true;
            _loc4_ = this.var_872[_loc3_];
         ...
         {
            delete this.var_872[_loc3_];
         }
         while(_loc6_ < this.var_715.length)
         {
            _loc5_ = this.var_715[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_715.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_873.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_714.length > 0 && this.var_715.length < this.const_1534)
         {
            _loc1_ = this.var_714[0];
            _loc1_.startDownloading();
            this.var_715.push(this.var_714.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_714.indexOf(param1) == -1 && this.var_715.indexOf(param1) == -1)
         {
