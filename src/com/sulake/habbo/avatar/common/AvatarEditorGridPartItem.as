package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_672:IFigurePartSet;
      ...
      private const const_1984:int = 13421772;
      
      private const const_1985:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_658 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1555 = Math.max(this.var_1555,_loc6_.colorLayerIndex);
            }
         }
         this.var_392 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_658 != null && !this.var_1557)
            {
               _loc3_ = this.var_658;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_672)
            {
               switch(this.var_672.clubLevel)
               {
                  case HabboClubLevelEnum.const_55:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_756;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_900;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1984;
         }
         else
         {
            this._window.color = this.const_1985;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_392.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_327[_loc6_] + "_" + FigureData.const_143;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_554 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1556)
               {
                  ...
                  this.var_1556 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_554.width,this.var_554.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_554.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_554.y;
               if(this.var_1557 && _loc2_.colorLayerIndex > 0)
               {
