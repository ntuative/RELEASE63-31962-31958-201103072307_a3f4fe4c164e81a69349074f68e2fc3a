package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2126 = new RoomThumbnailRenderer(param1);
         this.var_1094 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_170 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_170.visible = true;
         Util.hideChildren(this.var_170);
         this.refreshRoomName(this.var_170,this.var_110);
         if(this.var_110.ownerName != "" && this.var_110.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_170,!!this.var_110.event ? this.getEventCtg(this.var_110.categoryId) : this.getRoomCtg(this.var_110.categoryId));
         this.refreshRoomDesc(this.var_170,this.var_110);
         this.refreshExtraCont();
         this.refreshInfo(this.var_170,"trading_allowed",this.var_110.allowTrading);
         this.refreshInfo(this.var_170,"doormode_doorbell",this.var_110.doorMode == 1);
         this.refreshInfo(this.var_170,"doormode_password",this.var_110.doorMode == 2);
         Util.moveChildrenToColumn(this.var_170,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_170.height = Util.getLowestPoint(this.var_170);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_170,this.var_110.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_170.findChildByName("roomowner"));
         if(this.var_110.ownerName != "" && this.var_110.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_170.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1094.refreshTags(_loc1_,this.var_110.tags);
         ...
         this.var_2126.refreshThumbnail(_loc1_,this.var_110.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_271,Tabs.const_233,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
