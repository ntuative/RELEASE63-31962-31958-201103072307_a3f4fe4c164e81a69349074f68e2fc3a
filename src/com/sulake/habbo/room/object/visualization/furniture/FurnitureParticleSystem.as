package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_249:IRoomObjectSprite;
      ...
      private var var_1122:Number = 1.0;
      
      private var var_599:BitmapData;
      
      private var var_724:ColorTransform;
      
      private var var_1362:ColorTransform;
      
      private var var_1706:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_693:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_724 = new ColorTransform();
         this.var_724.alphaMultiplier = 1;
         this.var_1362 = new ColorTransform();
         this.var_1706 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_95 = this.var_1121[param1];
         this.var_884 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1705 >= 0)
         {
            this.var_249 = this._visualization.getSprite(this.var_1705);
            if(this.var_249 && this.var_249.asset)
            {
               if(this.var_249.width <= 1 || this.var_249.height <= 1)
               {
                  return;
               }
               if(this.var_86 == null)
               {
                  this.var_86 = this.var_249.asset.clone();
                  if(this.var_724.alphaMultiplier != 1)
                  {
                     this.var_599 = new BitmapData(this.var_86.width,this.var_86.height,true,4278190080);
                  }
               }
               this.var_2384 = -this.var_249.offsetX;
               this.var_2383 = -this.var_249.offsetY;
               this.var_249.asset = this.var_86;
            }
            if(this.var_86)
            {
            ...
            if(this.var_599)
            {
               this.var_599.fillRect(this.var_599.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_95.y * this.var_1122;
         }
         return 0;
      }
      ...
      {
         if(!this.var_95 || !this.var_249)
         {
            return;
         }
         if(this.var_86 && this.var_249.asset != this.var_86)
         {
            this.var_249.asset = this.var_86;
         }
         if(this.var_884)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_884 && this.var_95.hasIgnited)
         {
            this.var_884 = true;
         }
         _loc8_ = this.var_1115 * this.var_1122;
         this.var_95.update();
         if(this.var_884)
         {
            if(this.var_95.roomObjectSpriteId >= 0)
            ...
            {
               this.var_86.draw(this.var_599,this.var_1706,this.var_724,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_95.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2384 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1122;
               _loc4_ = this.var_2383 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1122;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1362.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_86.draw(_loc7_,this._translationMatrix,this.var_1362,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2382 = parseInt(param1.@size);
         this.var_1705 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1115 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1122 = this.var_2382 / 64;
         this.var_693 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_693 = Math.min(this.var_693,1);
         this.var_724.alphaMultiplier = this.var_693;
