package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1534:int = 4;
      ...
      {
         return this.var_434.length + this.var_274.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_274.length < const_1534 && this.var_434.length > 0)
            {
               _loc1_ = this.var_434.shift();
               this.var_274.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_434.indexOf(loader);
            if(index > -1)
            {
               this.var_434.splice(index,1);
            }
            index = this.var_274.indexOf(loader);
            if(index > -1)
            {
               this.var_274.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_434)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_274)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
