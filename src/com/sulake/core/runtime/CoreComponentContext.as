package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1291:String = "asset-libraries";
      
      private static const const_1290:String = "library";
      
      private static const const_1292:String = "service-libraries";
      
      private static const const_1295:String = "library";
      
      private static const const_1296:String = "component-libraries";
      
      private static const const_1294:String = "library";
      
      private static const const_1298:String = "error_data";
      
      private static const const_1293:String = "error_cat";
      
      private static const const_1297:String = "error_desc";
      ...
      private var var_340:Function;
      
      private var var_191:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_444.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1149:
               debug("Core; using simple frame update handler");
               this.var_340 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_789:
               debug("Core; using complex frame update handler");
               this.var_340 = this.complexFrameUpdateHandler;
               break;
            case Core.const_770:
               debug("Core; using profiler frame update handler");
               this.var_340 = this.profilerFrameUpdateHandler;
               this.var_191 = new Profiler(this);
               attachComponent(this.var_191,[new IIDProfiler()]);
               var_444.addChild(new ProfilerViewer(this.var_191));
               break;
            case Core.const_539:
               debug("Core; using experimental frame update handler");
               this.var_340 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_340 = this.debugFrameUpdateHandler;
         }
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            var_444.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
            var_444 = null;
            _loc3_ = 0;
            while(_loc3_ < CoreComponentContext.const_102)
            {
               _loc2_ = this.var_244[_loc3_] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         ...
         ErrorReportStorage.setParameter(const_1293,String(param3));
         ErrorReportStorage.setParameter(const_1297,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         try
         {
            node = config.child(const_1291)[0];
            if(node != null)
            {
               list = node.child(const_1290);
         ...
         try
         {
            node = config.child(const_1292)[0];
            if(node != null)
            {
               list = node.child(const_1295);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1296)[0];
            if(node != null)
            {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1379);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_711)
                  {
                     this.var_711.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_473 - this.var_339.length,this.var_473,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_191 ? 0 : this.var_1323 & 0;
         if(_loc3_ == Core.const_539)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_340(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var msCurrentTime:uint = param1;
         var msSinceLastUpdate:uint = param2;
         var maxTimePerFrame:uint = 1000 / var_444.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_102)
         {
            t = getTimer();
            ...
            {
               if(this.var_584[priority] < priority)
               {
                  ++this.var_584[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_102)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_191.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_191)
            {
               detachComponent(this.var_191);
               this.var_191.dispose();
               this.var_191 = null;
            }
            switch(this.var_1323 & 0)
            {
               case Core.const_1149:
                  this.var_340 = this.simpleFrameUpdateHandler;
                  break;
